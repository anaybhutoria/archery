...
const GameSVG = forwardRef(function GameSVG({ upgrades, targetRef }, svgRef) {
  const bowScale = 0.7 + upgrades.bigBow * 0.4
  const targetScale = 0.7 + upgrades.eagleEye * 0.4

  return (
    <svg ref={svgRef} id="game" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 400" overflow="visible">
      <defs>
        <linearGradient id="ArcGradient">
          <stop offset="0" stopColor="#fff" stopOpacity=".2" />
          <stop offset="50%" stopColor="#fff" stopOpacity="0" />
        </linearGradient>
        <g id="arrow" transform={`scale(${bowScale})`}>
          <line x2="60" fill="none" stroke="#888" strokeWidth="2" />
          <polygon fill="#888" points="64 0 58 2 56 0 58 -2" />
          <polygon fill="#8e44ad" points="2 -3 -4 -3 -1 0 -4 3 2 3 5 0" />
        </g>
      </defs>

      <path id="arc" fill="none" stroke="url(#ArcGradient)" strokeWidth="4"
        d="M100,250c250-400,550-400,800,0" pointerEvents="none" />

      <g ref={targetRef}>
        <g transform={`translate(${60 - (targetScale - 0.7) * 80},0) translate(900,250) scale(${targetScale}) translate(-900,-250)`}>
      ...
      <g transform={`translate(88,250) scale(${bowScale}) translate(-88,-250)`}>
        <g id="bow" fill="none" strokeLinecap="round" vectorEffect="non-scaling-stroke" pointerEvents="none">
          ...
          <path fill="none" stroke="#8e44ad" strokeWidth="3" strokeLinecap="round" d="M88,300 c0-10.1,12-25.1,12-50s-12-39.9-12-50" />
        </g>
        <g className="arrow-angle">
          <use x={100 - (bowScale - 1) * 50} y="250" href="#arrow" />
        </g>
      </g>

      <g className="arrows" pointerEvents="none" />
